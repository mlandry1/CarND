...
These random transforms are applied on images from all three dashboard cameras to generate 3 new images and then 3 other images are generated by flipping the augmented images in order to keep the generated data mean value to 0.
 
After the whole process, I had 65904 training sample and 28248 validation samples. 


I then preprocessed this data by ... TODO..
...
Best epoch wass #3.. with val_loss =0.0084 and train_loss= 0.0055.
 Reload the 
...
I had the idea of converting the color space to YUV as the article fom Nvidia used but I found not easy way to convert it inside the Keras model. It had to be implemented in the model since the conversion is needed during inference as well.
NVIDIA paper - section 4 https://arxiv.org/pdf/1604.07316.pdf

 Comma.ai's model from: https://github.com/commaai/research.
 
 I also tought of low-pass filter the steering angle data but since my dataset is made out of multiple "record on-record off".. it would make sense to filter this data. i would have had to separate each recording session and filter it individualy.
 
I noticed the shadows does'nt appear when the grpahic mode "fastest" is selected. I decided to give a try with the shadows. Therefore I used random shadowing of the training images in order to densisitize the network to those.


Gaussian Blur to smooth artifacts in the training images..

Blog post inspirants :


http://davidaventimiglia.com/carnd_behavioral_cloning_part1.html?fb_comment_id=1429370707086975_1432730663417646&comment_id=1432702413420471&reply_comment_id=1432730663417646#f2752653e047148

data flatening:?
in order to reduce the importance of data arround the 0 steering angle..
